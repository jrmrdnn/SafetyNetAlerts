...
class PersonServiceTest {
  ...
  @Nested
  class AllHouseholdMembers {

    @Test
    void shouldReturnHouseholdMembers() {
      List<Person> personsAtAddress = Arrays.asList(person1, person2);

      ChildAlertDTO childAlertDTO = new ChildAlertDTO();
      childAlertDTO.setFirstName("Child");
      childAlertDTO.setLastName("Test");

      ChildAlertDTO result = personService.allHouseholdMembers(
        personsAtAddress,
        child,
        childAlertDTO
      );

      assertFalse(result.getHouseholdMembers().isEmpty());
      assertEquals(2, result.getHouseholdMembers().size());
      assertEquals(
        "FirstName1",
        result.getHouseholdMembers().get(0).getFirstName()
      );
      assertEquals(
        "FirstName2",
        result.getHouseholdMembers().get(1).getFirstName()
      );
    }

    @Test
    void whenNoHouseholdMembers() {
      List<Person> personsAtAddress = Arrays.asList(child);
    ...
    @Test
    void whenTheChildNameChanges() {
      Person child2 = new Person();
      child2.setFirstName("Child");
      child2.setLastName("Name");
      ...
      assertEquals("Child", result.getHouseholdMembers().get(0).getFirstName());
    }
  }
}
